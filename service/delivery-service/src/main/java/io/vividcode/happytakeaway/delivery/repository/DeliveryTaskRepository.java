...
public class DeliveryTaskRepository {
  ...
  public Uni<List<DeliveryTaskInfo>> listTasks() {
    return this.client.query("SELECT * FROM " + TASK_TABLE_NAME)
        .execute()
        .map(rows -> StreamSupport.stream(rows.spliterator(), false)
            .map(this::extractRow).collect(Collectors.toList()));
  }

  public Uni<DeliveryTaskInfo> getTask(String taskId) {
    return this.client.preparedQuery("SELECT * FROM " + TASK_TABLE_NAME + " WHERE id = $1")
        .execute(Tuple.of(taskId))
        .flatMap(rows ->
            Uni.createFrom().optional(StreamSupport.stream(rows.spliterator(), false)
                .findFirst()
                .map(this::extractRow))
        );
  }
  ...
  public Uni<Void> createTask(String taskId) {
    return this.client.preparedQuery(
        "INSERT INTO " + TASK_TABLE_NAME + " (id, status) VALUES ($1, $2)")
        .execute(Tuple.of(taskId, DeliveryTaskStatus.CREATED.name()))
        .onItem().ignore().andContinueWithNull();
  }

  public Uni<Void> selectRider(String taskId, String riderId) {
    return SqlClientHelper.inTransactionUni(this.client, tx -> {
      Uni<RowSet<Row>> updateTask = tx
          .preparedQuery(
              "UPDATE " + TASK_TABLE_NAME
                  + " SET rider_id = $1, status = $2 WHERE id = $3")
          .execute(Tuple.of(riderId, DeliveryTaskStatus.SELECTED.name(), taskId));
      return Uni.combine().all()
          .unis(
              updateTask,
              this.updatePickupStatus(tx, taskId, riderId,
                  DeliveryPickupInvitationStatus.SELECTED),
              this.getExistingRiders(tx, taskId)
                  .flatMap(riders -> {
                    ...
                    return Uni.combine().all()
                        .unis(riders.stream().map(id ->
                            this.updatePickupStatus(tx, taskId, id,
  ...
  public Uni<Void> createPickupInvitations(String taskId, List<String> riderIds) {
    return this.client.preparedQuery(
        "INSERT INTO " + PICKUP_TABLE_NAME
            + " (task_id, rider_id, status) VALUES ($1, $2, $3)")
        .executeBatch(riderIds.stream()
            .map(riderId ->
  ...
  private Uni<List<String>> getExistingRiders(SqlClient client, String taskId) {
    return this.queryForRiders(client.preparedQuery(
        "SELECT rider_id FROM " + PICKUP_TABLE_NAME + " WHERE task_id = $1"
    ), taskId);
  }

  public Uni<List<String>> getAcceptedRiders(String taskId) {
    return this.queryForRiders(this.client.preparedQuery(
        "SELECT rider_id FROM " + PICKUP_TABLE_NAME + " WHERE task_id = $1 AND status = 'ACCEPTED'"
    ), taskId);
  }

  public Uni<Void> updatePickupStatus(String taskId, String riderId,
      DeliveryPickupInvitationStatus status) {
    return this.updatePickupStatus(this.client, taskId, riderId, status);
  }

  private Uni<Void> updatePickupStatus(SqlClient client, String taskId, String riderId,
      DeliveryPickupInvitationStatus status) {
  ...
  private Uni<List<String>> queryForRiders(PreparedQuery<RowSet<Row>> query, String taskId) {
    return query.execute(Tuple.of(taskId))
        .onItem().transform(
