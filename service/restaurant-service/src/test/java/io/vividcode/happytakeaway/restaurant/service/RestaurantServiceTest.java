...
class RestaurantServiceTest {
  ...
  void createRestaurant() {
    String id = this.restaurantService
  ...
  void getRestaurant() {
    ...
    RestaurantWithMenuItems restaurant = this.restaurantService
        .getRestaurant(TestHelper.getRestaurantRequest(id, 0));
  ...
  void updateRestaurant() {
    String id = this.restaurantService
        .createRestaurant(TestHelper.createRestaurantRequest());
    String newName = "new";
    UpdateRestaurantResponse updateRestaurant = this.restaurantService
        .updateRestaurant(TestHelper.updateRestaurantRequest(id, newName));
    assertThat(updateRestaurant)
        .extracting(UpdateRestaurantResponse::getName)
        .isEqualTo(newName);
  }
  ...
  void setActiveMenu() {
    String restaurantId = this.restaurantService
        .createRestaurant(TestHelper.createRestaurantRequest());
    String menuId = this.menuService
        .createMenu(TestHelper.createMenuRequest(restaurantId, false));
    String menuItemId = this.menuItemService
        .createMenuItem(TestHelper.createMenuItemRequest(restaurantId));
    this.menuService.associateMenuItems(TestHelper.associateMenuItemsRequest(restaurantId, menuId,
        Set.of(menuItemId)));
    this.restaurantService.setActiveMenu(TestHelper.setActiveMenuRequest(restaurantId, menuId));
    RestaurantWithMenuItems restaurant = this.restaurantService
        .getRestaurant(TestHelper.getRestaurantRequest(restaurantId, 10));
  ...
  void testMenuItemPagination() {
    String restaurantId = this.restaurantService
        .createRestaurant(TestHelper.createRestaurantRequest());
    String menuId = this.menuService
        .createMenu(TestHelper.createMenuRequest(restaurantId, true));
    int numberOfMenuItems = 21;
    Set<String> menuItemIds = IntStream.range(0, numberOfMenuItems).mapToObj(
        index -> this.menuItemService
            .createMenuItem(TestHelper.createMenuItemRequest(restaurantId)))
        .collect(Collectors.toSet());
    this.menuService
        .associateMenuItems(
            TestHelper.associateMenuItemsRequest(restaurantId, menuId, menuItemIds));
    PagedResult<MenuItem> result = this.restaurantService
        .getMenuItems(restaurantId, PageRequest.of(0, 10));
