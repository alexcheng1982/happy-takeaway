...
public class MenuItemService {
  ...
  public MenuItem getMenuItem(GetMenuItemRequest request) {
    return this.restaurantRepository
        ...
        .map(restaurant ->
            this.menuItemRepository.findById(request.getRestaurantId(), request.getMenuItemId())
                .map(
  ...
  public UpdateMenuItemResponse updateMenuItem(UpdateMenuItemRequest request) {
    return this.restaurantRepository
        .findByOwnerIdAndId(this.ownerIdProvider.get(), request.getRestaurantId())
        .map(restaurant -> this.menuItemRepository
        ...
        .orElseThrow(() -> ServiceHelper.restaurantNotFound(
  ...
  public PagedResult<MenuItem> findMenuItems(String menuId, PageRequest pageRequest) {
    ...
    long totalResults = this.entityManager
        .createQuery(countQuery
            .select(criteriaBuilder.count(countQueryRoot.join("items"))))
        .getSingleResult();

    CriteriaQuery<MenuItemEntity> itemsQuery = criteriaBuilder
        .createQuery(MenuItemEntity.class);
    ...
    List<MenuItemEntity> data = this.entityManager
        .createQuery(
            itemsQuery.select(itemsQueryRoot.join("items")).distinct(true))
        .setFirstResult(pageRequest.getOffset())
        .setMaxResults(pageRequest.getSize())
        .getResultList();

    List<MenuItem> results = ServiceHelper
        .transform(data, (entity) -> ServiceHelper.buildMenuItem(entity, this::getCoverImageUrl));
