...
public class DataLoaderApplication implements QuarkusApplication {
  ...
  public int run(String... args) throws Exception {
    Path dataPath = Paths
        .get(DataLoaderApplication.class.getResource("/test-data/restaurants").toURI());
    Files.newDirectoryStream(dataPath, entry -> entry.toFile().isDirectory())
        .forEach(restaurantPath -> {
          try {
            Restaurant restaurant = this.objectMapper
                .readValue(restaurantPath.resolve("restaurant.json").toFile(), Restaurant.class);
            ...
            logger.info("Create menu for restaurant {} with id {}", restaurant.getName(), menuId);
            Path menuItemsPath = restaurantPath.resolve("menuitems");
            Set<String> menuItemIds = new HashSet<>();
            Files.newDirectoryStream(menuItemsPath, entry -> entry.toFile().isDirectory()).forEach(
                menuItemPath -> {
                  try {
                    MenuItem menuItem = this.objectMapper
                        .readValue(menuItemPath.resolve("menuitem.json").toFile(), MenuItem.class);
                    UploadResult uploadResult = this.fileUploadServiceClient
                        .upload(FileUploadBody.builder()
                            .file(Files.newInputStream(menuItemPath.resolve("cover.jpg")))
                            .build());
                    String coverImage =
                        uploadResult.getFiles().length > 0 ? uploadResult.getFiles()[0].getPath()
                            : null;
                    String menuItemId = this.createMenuItem(restaurantId, menuItem, coverImage);
                    logger.info("Create menu item {} with id {}", menuItem.getName(), menuItemId);
  ...
  private String createRestaurant(Restaurant restaurant) {
    ...
    Response response = this.restaurantServiceClient
        .createRestaurant(CreateRestaurantWebRequest.builder()
  ...
  private String createMenu(String restaurantId) {
    Response response = this.restaurantServiceClient
        .createMenu(restaurantId, CreateMenuWebRequest.builder()
            .name("default")
            .current(true)
            .build());
    return this.extractId(response);
  ...
  private void associateMenuItems(String restaurantId, String menuId, Set<String> menuItemIds) {
    AssociateMenuItemsWebResponse response = this.restaurantServiceClient
        .associateMenuItems(restaurantId, menuId, new AssociateMenuItemsWebRequest(menuItemIds));
